...
def odes(r, y, p):
    """
    ODE for volume-charge microgels weakly charged

    r (float): grid in the radial axis, in units of a
    y (np.array(2)): y[0] electric potential, y[1] electric field.
    p (np.array(2)): equation parameters [kresa, k0_aa, xi_a, k0_ab, xi_b].
    """
    kresa, k0_aa, xi_a, k0_ab, xi_b = p
    dydr = np.zeros_like(y)
    dydr[0] = y[1]
    dydr[1] = f_diff_eq(r, y[0], y[1], kresa, k0_aa, xi_a, k0_ab, xi_b)
...
def boundary_conditions(ya, yb):
    """
    boundary conditions of the ODE for volume-charge microgels weakly charged
