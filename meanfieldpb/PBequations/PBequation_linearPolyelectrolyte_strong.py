...
def odes(r, y, kappa):
    """
    ODE for linear polyelectrolyte strongly charged, along z-axis
    with poyelectrolyte radius a.

    r (float): grid in the radial axis, in units of a
    y (np.array(2)): y[0] electric potential, y[1] electric field.
    kappa (float): screening constant in units of a.
    """
...
def boundary_conditions(ya, yb, xi):
    """
    boundary conditions of the ODE for linear polyelectrolyte strongly charged,
    along z-axis with poyelectrolyte radius a.
    ...
    xi (float): dimensionless linear charge density in units of a.
    """
    phi_prime_a = 2 * xi  # Boundary condition at r=a
    phi_prime_R = 0.0  # Boundary condition at r=R
    return np.array([ya[1] - phi_prime_a, yb[1] - phi_prime_R])
