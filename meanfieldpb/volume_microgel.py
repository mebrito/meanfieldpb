...
class VolumeMicrogel(Suspension, WeakParticle):
    ...
    def __init__(
        self, a, Z_a, Z_b, lb, vol_frac, c_salt, charge_type, pK_a=4, pK_b=4, pH_res=7
    ):
        ...
        if self.charge_type == "strong":
            ...
            self.linear_solution = LinearVolumeMicrogelWeak(
                self.a,
                self.lb,
                self.vol_frac,
                c_salt,
    ...
    def set_grid(self, r: np.array):
        ...
        assert self.r[-1] <= self.R_cell, (
            "Last component of the grid smaller than R_cell"
        )
    ...
    def solve_nonlin_PB(self, r, y_init):
        ...
        if self.charge_type == "strong":
            ...
            params = np.array(
                [self.kresa, self.k0_aa, self.xi_a, self.k0_ab, self.xi_b]
            )
            solution = solve_bvp(
                lambda r, y: PBweak.odes(r, y, params),
                lambda ya, yb: PBweak.boundary_conditions(ya, yb),
                r,
                y_init,
            )
