...
def variables(a, lb, vol_frac, c_salt, Zmax_a, Zmax_b, pKa, pKb, pH, c_ref=1):
    ...
    R = (a**3 / vol_frac) ** (1.0 / 3.0)  # cell radius
    c_Hres = c_ref * 10 ** (-pH)  # [M] # H^+ reservoir concentration
    c_res = reservoir_concent(
        c_Hres, c_salt, c_ref
    )  # total reservoir microion concentration
    ...
    c0acid_tilde = (
        3 * Zmax_a / (4 * math.pi * a**3)
    )  # concentration of acidic sites in number of particles per nm^3
    k_acid = np.sqrt(
        4 * math.pi * lb * c0acid_tilde * KaacidCref / (c_Hres + KaacidCref)
    )
    h_acid = c_Hres / (c_Hres + KaacidCref)

    KabasicCref = KaCref(pKb, c_ref)
    c0basic_tilde = (
        3 * Zmax_b / (4 * math.pi * a**3)
    )  # concentration of acidic sites in number of particles per nm^3
    k_basic = np.sqrt(
        4 * math.pi * lb * c0basic_tilde * c_Hres / (c_Hres + KabasicCref)
    )
...
class LinearVolumeMicrogelWeak:
    ...
    def __post_init__(self):
        R, c_Hres, k_res, k_acid, h_acid, k_basic, h_basic = variables(
            self.a,
            self.lb,
            self.vol_frac,
            self.c_salt,
        ...
        self.AA = (
            self.c
            * (
                math.exp(2 * R * k_res) * (1 + self.a * k_res) * (-1 + R * k_res)
                - math.exp(2 * self.a * k_res) * (-1 + self.a * k_res) * (1 + R * k_res)
            )
            * csch(self.a * self.zeta)
        ) / (
            (-math.exp(2 * self.a * k_res))
            * (1 + R * k_res)
            * (k_res - self.zeta * coth(self.a * self.zeta))
            + math.exp(2 * R * k_res)
            * (-1 + R * k_res)
            * (k_res + self.zeta * coth(self.a * self.zeta))
        )
        ...
        self.BB = (
            self.c
            * math.exp(self.a * k_res)
            * (1 + R * k_res)
            * (
                self.a * self.zeta * math.cosh(self.a * self.zeta)
                - math.sinh(self.a * self.zeta)
            )
        ) / (
            math.exp(2 * self.a * k_res)
            * (1 + R * k_res)
            * (
                (-self.zeta) * math.cosh(self.a * self.zeta)
                + k_res * math.sinh(self.a * self.zeta)
            )
            - math.exp(2 * R * k_res)
            * (-1 + R * k_res)
            * (
    ...
    def lin_elec_pot(self, r):
        ...
        def outer(x):
            return (
                self.BB * np.exp(self.k_res * x) / x
                + self.CC * np.exp(-self.k_res * x) / x
            )

        return np.piecewise(
            r, [r <= self.a, self.a < r], [lambda r: inner(r), lambda r: outer(r)]
        )
