...
from meanfieldpb.Swelling.polymer_network import (
    PolymerNetwork,
    CrosslinkedPolymerNetwork,
    UncrosslinkedPolymerNetwork,
    FEPolymerNetwork,
)
...
def crosslinked_polymer_contribution(network: CrosslinkedPolymerNetwork):
    return pressures.pi_g_crosslinked(
        network.a,
        network.a0,
        network.N_m,
        network.Nch,
        network.Nmonch,
        network.chi_parameter,
    )


def uncrosslinked_polymer_contribution(network: UncrosslinkedPolymerNetwork):
    return pressures.pi_g(
        network.a,
        network.Nmonch,
        network.Nch,
        network.a0,
...
def fe_polymer_contribution(network: FEPolymerNetwork):
    return pressures.pi_g_Langevin(
        network.a,
        network.Nmonch,
        network.Nch,
        network.a0,
...
class OsmoticPressureCalculator:
    ...
    def __post_init__(self):
        if not isinstance(self.microgel_suspension, (VolumeMicrogel, SurfaceMicrogel)):
            raise TypeError(
                "microgel_suspension must be an instance of VolumeMicrogel or SurfaceMicrogel"
            )
    ...
    def calculate_polymer_contribution(self):
        ...
        if isinstance(self.network, CrosslinkedPolymerNetwork):
            ...
            raise TypeError(
                "network must be an instance of CrosslinkedPolymerNetwork, UncrosslinkedPolymerNetwork, or FEPolymerNetwork"
            )
