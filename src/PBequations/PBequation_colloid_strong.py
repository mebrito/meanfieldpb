...
def f_diff_eq(r, y, y_prime, kresa, Zlba):
    """
    Function that defines the second derivative in the differential
    equation y'' = f(r, y, y'; kresa, Zlba)

    All quantities should be in reduced units.
    """
    return -2 * y_prime / r + kresa * kresa * np.sinh(y)
...
def odes(r, y, p):
    ...
    dydr[0] = y[1]
    dydr[1] = f_diff_eq(r, y[0], y[1], kresa, Zlba)
    return dydr
...
def boundary_conditions(ya, yb, p):
    ...
    """
    kresa, Zlba = p
    phi_prime_1 = -Zlba  # Boundary condition at r=1
    phi_prime_R = 0.0  # Boundary condition at r=R
    return np.array([ya[1] - phi_prime_1, yb[1] - phi_prime_R])
